...
class PersonIdAudio:
    ...
    def get_tf_dataset(self):
        ...
        audio_dataset = tf.data.Dataset.from_generator(
            self.gen_audios, output_signature=audio_signature
        )
    ...
    def save_tfrecords_file(self, output_file, compression_type="GZIP"):
        ...
        audio_dataset = audio_dataset.map(PersonIdAudio._serialize_for_tfrecords)
...
class AudioTarReader:
    ...
    def retrieve_per_user_data(self, split="train"):
        ...
        path_to_client_dict = dict(
            zip(self.data_files[split].path, self.data_files[split].client_id)
        )
        audio_content = {}
...
def parse_args():
    ...
    parser.add_argument(
        "folder", type=str, help="Folder containing Mozilla Speech files to be parsed"
    )
...
def main():
    ...
    sr = 48000
    assert os.path.isdir(args.folder), f"Folder does not exit: {args.folder}"
    ...
    for file in files:
        ...
        for split in ["train", "dev", "test"]:
            ...
            if not os.path.isfile(target_name):
                ...
                tfrecords_file = pia.save_tfrecords_file(target_name)
